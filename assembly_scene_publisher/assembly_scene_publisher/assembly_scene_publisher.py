...
class TFPublisherNode(Node):
    def __init__(self):
        ...
        self.spawn_object_srv = self.create_service(ami_srv.SpawnObject,f'object_spawner_publisher/spawn_object',self.spawn_object_callback,callback_group=self.callback_group)
        self.destroy_object_srv = self.create_service(ami_srv.DestroyObject,f'object_spawner_publisher/destroy_object',self.destroy_object_callback,callback_group=self.callback_group)

        self.create_ref_frame_srv = self.create_service(ami_srv.CreateRefFrame,f'object_spawner_manager/create_ref_frame',self.create_ref_frame,callback_group=self.callback_group)
        self.delete_ref_frame_srv = self.create_service(ami_srv.DeleteRefFrame,f'object_spawner_manager/delete_ref_frame',self.destroy_ref_frame,callback_group=self.callback_group)  

        self.change_parent_frame_srv = self.create_service(ami_srv.ChangeParentFrame,f'object_spawner_manager/change_obj_parent_frame',self.change_obj_parent_frame,callback_group=self.callback_group)  
        
        self.change_parent_frame_srv = self.create_service(ami_srv.ModifyPose,'object_spawner_manager/modify_pose',self.modify_pose,callback_group=self.callback_group)  

        self.get_info_srv = self.create_service(ami_srv.GetScene,f'object_spawner_manager/get_scene',self.get_scene,callback_group=self.callback_group)      

        self.create_ref_plane_srv = self.create_service(ami_srv.CreateRefPlane,f'object_spawner_manager/create_ref_plane',self.create_ref_plane,callback_group=self.callback_group)      
        
        self.create_axis_srv = self.create_service(ami_srv.CreateAxis,f'object_spawner_manager/create_axis',self.create_axis,callback_group=self.callback_group)      

        self.create_assembly_instructions_srv = self.create_service(ami_srv.CreateAssemblyInstructions,f'object_spawner_manager/create_assembly_instructions',self.srv_create_assembly_instructions,callback_group=self.callback_group)      

        self.calculate_assembly_instructions_srv = self.create_service(ami_srv.CalculateAssemblyInstructions,f'object_spawner_manager/calculate_assembly_instructions',self.calculate_assembly_instructions,callback_group=self.callback_group)      
